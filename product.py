...
# Load dataset
data = pd.read_csv('amazon.csv')  # Adjust the path if needed
...
def get_recommendations(product_name):
    ...
    # Get the scores of the 8 most similar products
    sim_scores = sim_scores[1:6]

    # Get the product indices
    product_indices = [i[0] for i in sim_scores]

    # Return the top 8 most similar products with their prices and reviews
    return data.iloc[product_indices][['product_name', 'discounted_price', 'review_content', 'discount_percentage', 'rating', 'user_id', 'product_link', 'about_product', 'user_name']]
...
if __name__ == "__main__":
    ...
    if recommendations is not None:
        ...
        for index, row in recommendations.iterrows():
            ...
            print(f"Productlink: {row['product_link']}")
            print(f"AboutProduct: {row['about_product']}")
